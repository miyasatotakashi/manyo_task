...
RSpec.describe 'タスクモデル機能', type: :model do    
  ...
  describe  '検索機能' do
    let!(:task) { FactoryBot.create(:task, title: 'test', status: '未着手') }
    ...
    context 'scopeメソッドでタイトルのあいまい検索とステータス検索をした場合' do
      it "検索キーワードをタイトルに含み、かつステータスに完全一致するタスク絞り込まれる" do
        expect(Task.title_search('test').status_search('未着手')).to include(task)
        expect(Task.title_search('test').status_search('未着手')).not_to include(second_task)
        expect(Task.title_search('test').status_search('未着手').count).to eq 1
      end
    end
  end
end
